...
//parse option
		...
		bool coapServer::loop() {
			...
			coapPacket receivedPacket;

			if (packetlen > 0) {
				packetlen = Udp.read(buffer, packetlen >= BUF_MAX_SIZE ? BUF_MAX_SIZE : packetlen);

				receivedPacket.bufferToPacket(buffer,packetlen);
		...
		//response
				if(receivedPacket.type_()==COAP_ACK){
					// call responseACK function
					resp(&receivedPacket, Udp.remoteIP(), Udp.remotePort(), 0);
				}
...
//get request
uint16_t coapServer::get(IPAddress ip, int port, char *url) {
	send(ip, port, url, COAP_CON, COAP_GET, NULL, 0, NULL, 0,0,NULL);
}

//post request
uint16_t coapServer::post(IPAddress ip, int port, char *url, char *payload, int payloadlen) {
	send(ip, port, url, COAP_CON, COAP_POST, NULL, 0, (uint8_t *)payload, payloadlen,0,NULL);
}

uint16_t coapServer::send(IPAddress ip, int port, char *url, COAP_TYPE type, COAP_METHOD method, uint8_t *token, uint8_t tokenlen, uint8_t *payload, uint32_t payloadlen,uint8_t number,uint8_t optionbuffer) {
	...
	packet.messageid = rand();
	 ...
	 if(method!=COAP_EMPTY){
		...
		char* copy_url;
		char *sub_url; 
		char *delim_p = "/"; //uri-path deliminiter
		char *last_sub_url;
		...
		sub_url = strtok(copy_url, delim_p);
		...
		// check for existing query (just one query)
